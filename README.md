...
- `ADMIN_IP_FILTER`: JSON array of allowed IP addresses/CIDRs for admin access, such as `["203.0.113.10/32"]`. This is the base value for Terraform's `admin_ip_filter` variable. Each plan and apply workflow run appends its ephemeral GitHub Actions runner IPv4 as a `/32` and IPv6 as a `/128` before OpenTofu initializes, while preserving the configured entries.

...
- The root module composes `network`, `cluster`, `cache`, and `database`; Flux bootstrap runs after the cluster is available.
...
- Format and validate with `tofu fmt -check`, `tofu init -backend=false -input=false`, and `tofu validate -no-color`.
...
- `var.environment` selects `clusters/<environment>/flux-system/flux-instance.yaml`; a matching manifest must exist before applying that environment.
